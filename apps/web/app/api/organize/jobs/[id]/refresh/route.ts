...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  const result = await refreshOrganizeMetadataProviders(params.id, providers, { force: true });
  ...
  return ok({
    ...result,
    enabled: !disabled,
    message: disabled ? disabledMessages.join('；') || '外部数据查询或 AI 识别尚未配置。' : `已刷新，新增 ${result.added} 条候选建议。`
  });
}
